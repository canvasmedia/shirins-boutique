...
## 13. SPECIFIC LAYOUT FIXES

**Mobile Button Pairs (Always Side-by-Side)**
When two CTA buttons are placed together, prevent them from wrapping or causing horizontal scroll on tiny screens (≤380px).
- **Global CSS:** Add `overflow-x: hidden` to `html` and `body` (with `width: 100%` on body) to prevent side clipping.
- **Container Safety:** Ensure `.container` has `box-sizing: border-box`.
- **Button Group Utility:** Always use a dedicated wrapper class (e.g., `.btn-group`) with `display: flex; flex-direction: row; flex-wrap: nowrap;`. Ensure children have `flex: 0 0 auto; white-space: nowrap;`.
- **Mobile Breakpoints:**
  - **≤720px:** Reduce button padding (e.g., `12px 20px`), gap (`10px`), and ensure `.btn-group { max-width: 100%; }` and children have `min-width: 0;`.
  - **≤380px (iPhone SE, small Android):** Further reduce container padding (e.g., `16px`), button padding (e.g., `11px 16px`), and gap (`8px`).
- **Never do this:** Do not use `flex-wrap: wrap` for side-by-side CTA pairs.

---

## 14. PRE-FLIGHT CHECKLIST (MUST PASS before delivery)
...
## 15. ONE-LINE SUMMARY FOR THE AGENT
